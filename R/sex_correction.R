...
#' @return The function corrects sex in samples directly in the database and returns a xlsx file with the corrected samples.
#' @export
sex_correction <- function(data_connection,
                           start_year,
                           end_year,
                           program,
                           ocean,
  ...
  observe_sex_sql <- paste(readLines(con = system.file("sql",
                                                       "observe_sex.sql",
                                                       package = "codama"
  )), collapse = "\n")
  observe_sex <- dplyr::tibble(DBI::dbGetQuery(
    conn = data_connection[[2]],
    statement = observe_sex_sql
  ))
  # 3 - Query creation ----
  # Select the topiaid of the sex to be corrected and the topiaid of the new one
  samples_to_be_corrected <- samples_with_sex
    ...
    for (i in seq_len(length(set_id_list))) {
      ...
      for (j in seq_len(length(sp_list_i))) {
        ...
        for (k in seq_len(length(samplemeasure_id_set_i_sp_j))) {
          ...
          queries[[ct]] <- paste(
            "UPDATE ps_observation.samplemeasure SET sex = NULL, topiaversion = topiaversion+1, lastupdatedate = '",
            format(Sys.time(), "%Y-%m-%d %H:%M:%OS3"),
            "' WHERE topiaid = '",
            sample_set_i_sp_j_sm_k$samplemeasure_id,
            "' AND sex = '",
            as.character(observe_sex[observe_sex$sex == sample_set_i_sp_j_sm_k$sex, "sex_id"]),
            "';",
            sep = ""
          )
        }
        ct <- ct + 1
        queries[[ct]] <- paste("UPDATE ps_observation.sample SET comment = concat(comment,'[Correction sexe ",
                               observe_sex$sex[observe_sex$sex == sample_set_i_sp_j$sex[[k]]],
                               " en sexe NULL des echantillons ",
                               sp_list_i[j],
                               " - ",
                               date,
                               " - ",

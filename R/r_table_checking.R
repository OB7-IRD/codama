...
#' @param output {\link[base]{character}} expected. Kind of expected output. You can choose between "message" or "logical".
#' @return The function returns a {\link[base]{character}} with output is "message", a {\link[base]{logical}} with output is "logical"
#' @export
r_table_checking <- function(r_table,
  ...
  if (r_type_checking(
    r_object = output,
    type = "character",
    allowed_value = c("message", "logical"),
    output = "logical"
  ) != TRUE) {
    return(r_type_checking(
      r_object = output,
  ...
  if (sum(checks_column_names) != length(column_name)) {
    if (output == "message") {
      stop(cat(
        ...
        paste0(" Error - invalid data, the following column names in \"column_name\" were not found in \"r_table\" : ", paste0(column_name[!checks_column_names], collapse = ", "), "\n")
      ))
    }
    if (output == "logical") {
      return(FALSE)

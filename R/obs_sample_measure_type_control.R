...
#' #Sample 4 is not ok: the sample is a FAL measured in FL
#' sample <- data.frame(samplemeasure_id = c("1", "2", "3", "4"),
#'                      sizetype_code = c("TL", "TL", "SCL", "FL"),
#'                      defaultsizetype_code = c("TL", "TL", "SCL", "TL"))
#' @expect equal(., structure(list(samplemeasure_id = c("1", "2", "3", "4"), sizetype_code = c("TL", "TL", "SCL", "FL"), defaultsizetype_code = c("TL", "TL", "SCL", "TL"), logical = c(TRUE, TRUE, TRUE, FALSE)), class = "data.frame", row.names = c(NA, -4L)))
...
obs_sample_measure_type_control <- function(sample) {
  ...
  if (!codama::r_table_checking(
    r_table = sample,
    type = "data.frame",
    column_name = c("samplemeasure_id", "sizetype_code", "defaultsizetype_code"),
    ...
    sample <- sample[, c("samplemeasure_id", "sizetype_code", "defaultsizetype_code")]
  }
  # 2 - Data manipulation ----
  sample_measure_type <- sample %>%
    dplyr::mutate(logical = (sizetype_code == defaultsizetype_code))
  # 3 - Return ----

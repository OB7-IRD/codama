#' @name check_specie_catch_ocean
#' @title Gives the inconsistencies between the species fished and the ocean reported for the trip
#' @description The purpose of the check_specie_catch_ocean function is to provide a table of data that contains an inconsistency between the species caught during the trip and the ocean reported for the trip with the ocean for which the species are associated
#' @param data_connection {\link[base]{list}} expected. Either output of the function {\link[furdeb]{postgresql_dbconnection}}, which must be done before using the check_specie_catch_ocean function or the two data sets used (the first one is the species-ocean associations and the second one is the species caught with the associated ocean)

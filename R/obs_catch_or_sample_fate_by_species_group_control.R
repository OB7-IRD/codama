...
#'                     speciesgroup_label1 = c("Cetaceans", "Other bony fishes", "Cetaceans", "Tunas nei", "Other bony fishes", "Tunas nei", "Tunas nei", "Other bony fishes"))
#' @expect equal(., structure(list(catch_id = c("1", "2", "3", "4", "5", "6", "7", "8"), speciesfate_code = c("1", "2", "4", "6", "6", "15", "15", "10"), species_faocode = c("MYS", "ALM", "MYS", "YFT", "CNT", "LTA", "BET", "MOP"), speciesgroup_label1 = c("Cetaceans", "Other bony fishes", "Cetaceans", "Tunas nei", "Other bony fishes", "Tunas nei", "Tunas nei", "Other bony fishes"), logical = c(TRUE, FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, FALSE)), class = "data.frame", row.names = c(NA, -8L)))
...
obs_catch_or_sample_fate_by_species_group_control <- function(data) {
  ...
  speciesgroup_label1 <- NULL
  # 1 - Arguments verification ----
  if (!(codama::r_table_checking(r_table = data,
                                 type = "data.frame",
                                 column_name = c("catch_id", "speciesfate_code", "species_faocode", "speciesgroup_label1"),
                                 column_type = c("character", "character", "character", "character"),
                                 output = "logical") ||
        codama::r_table_checking(r_table = data,
                                 type = "data.frame",
                                 column_name = c("samplemeasure_id", "speciesfate_code", "species_faocode", "speciesgroup_label1"),
                                 column_type = c("character", "character", "character", "character"),
                                 output = "logical"))) {
    ...
    stop(
      ...
      "\n  'catch_id' (character), 'speciesfate_code' (character), 'species_faocode' (character), 'speciesgroup_label1' (character)",
      "\n  or",
      "\n  'samplemeasure_id' (character), 'speciesfate_code' (character), 'species_faocode' (character), 'speciesgroup_label1' (character)",
      ,
      sep = ""
    )
  } else {
    if ("catch_id" %in% colnames(data)) {
      data <- data[, c("catch_id", "speciesfate_code", "species_faocode", "speciesgroup_label1")]
    }
    if ("samplemeasure_id" %in% colnames(data)) {
      data <- data[, c("samplemeasure_id", "speciesfate_code", "species_faocode", "speciesgroup_label1")]
    }
  }
  # 2 - Data manipulation ----
  ...
  data_fate_by_species_group_pb <- data %>%
    dplyr::filter(
      ...
      (speciesfate_code %in% c(1, 2, 3) & !(speciesgroup_label1 %in% c("Cetaceans", "Whale shark"))) |
        # Whale sharks and whales can only have codes 1, 2 or 3
        (speciesgroup_label1 %in% c("Cetaceans", "Whale shark") & !(speciesfate_code %in% c(1, 2, 3))) |
        ...
        (speciesfate_code == 15 & (!(speciesgroup_label1 %in% c("Billfishes", "Other bony fishes") | (speciesgroup_label1 == "Tunas nei" & !(species_faocode %in% c("YFT", "BET", "SKJ", "ALB")))))) |
        # Code 10 ("ailerons seulement") is only for sharks
        (speciesfate_code == 10 & speciesgroup_label1 != "Sharks")
    )
  if ("catch_id" %in% colnames(data)) {
